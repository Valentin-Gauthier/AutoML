...
class AutoML:
    ...
    def __init__(self):
        ...
        with open("src/nevergrad/models_config.yaml", 'r') as f:
        ...
        self.executor = submitit.AutoExecutor(folder="automl_logs")

        self.executor.update_parameters(
            timeout_min=60,      # 1 heure max par job 
            mem_gb=4,            # 4 Go de RAM par job 
            cpus_per_task=1,     # 1 CPU par job (car on parallélise les jobs) 
            slurm_partition="gpu"
        )
    ...
    def _config_to_nevergrad(self, grid_config):
        """
        Transforme votre config YAML (listes) en Espace de Recherche Nevergrad.
        Ex: [10, 100] devient ng.p.Choice([10, 100])
        """
        ng_dict = {}
        for param, values in grid_config.items():
            if isinstance(values, list):
                # Si c'est une liste, Nevergrad doit choisir dedans
                ng_dict[param] = ng.p.Choice(values)
            else:
                # Si c'est une valeur fixe
                ng_dict[param] = values
        
        # Instrumentation crée l'espace de recherche complet
    ...
    def fit(self, folder:str, test_size:float=0.2, verbose:bool=False):
        ...
        # =========================================================================
        # 3. Optimisation Distribuée (REMPLACEMENT DE GRIDSEARCHCV)
        # =========================================================================
        
        # A. Filtrage des modèles
        models_to_test = self._filter_models(X_train, y_train, verbose=verbose)
        
        if verbose:
            print(f"\n=======================================================")
            print(f" LANCEMENT OPTIMISATION DISTRIBUÉE (Cluster Skinner)")
            print(f" Modèles à optimiser : {len(models_to_test)}")
            print(f"=======================================================\n")

        for model_name, model_instance in models_to_test:
            
            # On vérifie si on a une config pour ce modèle dans le YAML
            if model_name not in self.param_grids:
                # Si pas de config, on entraîne juste le modèle par défaut localement
                print(f" [LOCAL] {model_name} (Pas de grille d'hyperparamètres)")
                model_instance.fit(X_train, y_train)
                self.trained_models[model_name] = model_instance
                continue

            try:
                print(f" [CLUSTER] Optimisation de : {model_name} ...")

                if any(x in model_name for x in ["MLP", "Random Forest", "Gradient Boosting", "SVR"]):
                    self.executor.update_parameters(
                        mem_gb=8,  
                        cpus_per_task=1, 
                        timeout_min=120  
                ...
                # 1. Préparer l'ouvrier (Trainer)
                # On envoie la CLASSE (ex: RandomForestClassifier) et les données
                # L'ouvrier fera le cross-validation sur le cluster
                ...
                # 2. Préparer l'espace de recherche (YAML -> Nevergrad)
                param_space = self._config_to_nevergrad(self.param_grids[model_name])
                
                # 3. Préparer l'optimiseur
                # budget=20 signifie qu'on va tester 20 combinaisons différentes
                optimizer = ng.optimizers.TwoPointsDE(parametrization=param_space, budget=200, num_workers=20)

                # 4. LANCEMENT !
                # .minimize va utiliser l'executor pour envoyer les jobs au cluster
                # batch_mode=True permet d'envoyer plusieurs jobs en même temps
                recommendation = optimizer.minimize(trainer, executor=self.executor, batch_mode=True)
                
                # 5. Récupération du résultat
                best_params = recommendation.value[1] # .kwargs
                print(f"    -> Fin. Meilleurs params : {best_params}")

                # 6. Ré-entraînement FINAL (Local)
                # On prend les meilleurs paramètres trouvés par le cluster
                # Et on entraîne un modèle final sur TOUT le jeu d'entraînement (pas de CV)
                ...
                # En cas d'erreur cluster, on essaie quand même le modèle par défaut
                try:
                    model_instance.fit(X_train, y_train)
                    self.trained_models[model_name] = model_instance
                except:
